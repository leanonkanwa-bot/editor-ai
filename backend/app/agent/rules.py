...
# MOTION_GRAPHICS — DEAD CODE: this constant is never imported or used anywhere.
# The planner's `motion_graphics` field is deprecated (storyboard.py OUTPUT_CONTRACT).
# Graphic cards are generated by storyboard.py with its own inline prompt.
# Kept here for historical reference only — do NOT include in any prompt.
MOTION_GRAPHICS = """\
...
MOMENTUM_STYLE = """\
MOMENTUM EDITING STYLE — ACTIVE
Target audience: business owners, coaches, entrepreneurs, finance-curious creators.
Energy: 8/10 — fast, punchy, relentless forward drive.

PACING RULES:
- Visual change every 1.2–2.0 seconds in the hook section
- Visual change every 2.0–2.8 seconds in body sections
- Absolute maximum: 3.5 seconds without visual change
- Mean shot duration: 1.8 seconds

SILENCE REMOVAL (aggressive):
- Cut every pause longer than 0.3s down to 0.05–0.1s
- Never leave a gap longer than 0.7s anywhere in the edit
- Remove filler words (um, uh, like, you know, basically) and breathing entirely

CUT PLACEMENT:
- Cut on the beat — start of the next stressed word, never mid-word
- Hard cuts only — no cross-dissolves except emotional beats (per BROLL_RULES)
- Every new idea or sentence triggers a new shot or zoom change

HOOK STRUCTURE (first 3 seconds — MANDATORY):
- Open on the boldest claim or number from the entire transcript
- Immediate jump-zoom to 150% on the first word
- Caption appears within the first frame — no empty screen time
...
- portrait_callout: black & white headshot with a tilted red label banner
  (name + role/claim) — use when introducing a person or named example
- step_diagram: dark background, glowing center icon, "STEP 0X" label —
...
- Duration 2.0–3.0s per clip, cut in on the anchor word, return to speaker immediately

ZOOM LIMITS:
- Maximum zoom: 150% (never exceed)
- Beat-mapped: hook/payoff/tension_stack=150%, amplify/tension=140%,
  realization=135%, principle=125%, story/emotional_end=120%, context=110%
...
def system_prompt(
    ...
    if editing_style == "priestley":
        ...
        blocks.insert(0, MOMENTUM_STYLE)
