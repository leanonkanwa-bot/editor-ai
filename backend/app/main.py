...
def _on_startup() -> None:
    ...
    # Minimal SIGTERM handler — rejette les nouvelles requêtes, exit immédiat.
    # Drain retiré (2026-08-31) pour la phase de tests intensifs : pushs instantanés.
    # ⚠ REMETTRE LE DRAIN AVANT TOUT TRAFIC UTILISATEUR RÉEL : voir memory/project_sigterm-drain.md
    _uvicorn_sigterm = _signal.getsignal(_signal.SIGTERM)

    def _sigterm_handler(signum: int, frame: object) -> None:
        _pipeline_shutdown.set()
        print("[SHUTDOWN] SIGTERM received — new renders rejected; exiting immediately", flush=True)
...
def _oauth_redirect_uri(request: Request) -> str:
    ...
    print(f"[OAUTH] redirect_uri={uri!r}  (APP_BASE_URL={settings.app_base_url!r})")
    return uri
...
def upsell_page(request: Request, job: str = "") -> Response:
    ...
    html = (
        ...
        '<div class="card">'
        f'<h1>{greeting}</h1>'
        '<p class="sub">Ta première vidéo est prête. Voilà ce que tu peux faire avec 15 vidéos par mois.</p>'
        f'<a href="{download_href}" class="dl-btn" download>Télécharger ma vidéo →</a>'
        '<hr class="divider">'
        '<p class="seen">Plan Starter - 29€/mois</p>'
        '<ul class="features">'
