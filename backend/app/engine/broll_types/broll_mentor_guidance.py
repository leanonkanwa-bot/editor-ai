...
def _extractor(match, words, word_idx: int) -> tuple[dict, float]:
    ...
    mentor_tokens = [getattr(words[i], "text", "") for i in range(word_idx, end)]
    mentor_fragment = " ".join(_merge_apos(mentor_tokens)).strip()
...
def _render_html(params: dict, pack: dict, card_id: str, compact: bool = False, layout: str = "portrait") -> str:
    ...
    pad        = "12px 18px" if compact else "28px 36px"
    text_size  = "15px"      if compact else "18px"
    lbl_size   = "9px"       if compact else "10px"
    lbl_margin = "4px"       if compact else "6px"
    conn_h     = "24px"      if compact else "32px"
    bubble_pad = "10px 14px" if compact else "14px 18px"
    ...
    if pack_id == "lean_glass":
        ...
        connector_style  = f"border-left:2px solid {accent}; margin-left:20px; height:0; opacity:0;"
        ...
        connector_style  = f"border-left:3px solid {accent}; margin-left:20px; height:0; opacity:0; border-radius:2px;"
    elif pack_id == "lean_ledger":
        bubble_mentor_bg = "rgba(0,200,150,0.12)"
        bubble_mentee_bg = "rgba(0,200,150,0.06)"
        connector_style  = f"border-left:1px solid {accent}; margin-left:16px; height:0; opacity:0;"
    elif pack_id == "lean_craft":
        bubble_mentor_bg = "rgba(217,119,87,0.12)"
        bubble_mentee_bg = "rgba(61,43,31,0.06)"
        connector_style  = f"border-left:2px dashed {accent}; margin-left:18px; height:0; opacity:0;"
    else:  # cinema
...
.card[data-card-id="{card_id}"] .mg-wrap{{background:{bg};border-radius:{radius};padding:{pad};
  display:flex;flex-direction:column;gap:0;box-shadow:{shadow_v};width:90%;max-width:440px{border_css};}}
.card[data-card-id="{card_id}"] .mg-section-label{{font-family:{font};font-size:{lbl_size};font-weight:700;
  letter-spacing:0.18em;text-transform:uppercase;color:{text_s};margin-bottom:{lbl_margin};opacity:0;}}
.card[data-card-id="{card_id}"] .mg-bubble{{background:{bubble_mentor_bg};border-radius:{bubble_radius};
  padding:{bubble_pad};opacity:0;}}
.card[data-card-id="{card_id}"] .mg-bubble-mentee{{background:{bubble_mentee_bg};border-radius:{bubble_radius};
  padding:{bubble_pad};opacity:0;}}
.card[data-card-id="{card_id}"] .mg-text{{font-family:{font};font-size:{text_size};font-weight:{fw};
  color:{text_c};line-height:1.35;{glow_css}}}
.card[data-card-id="{card_id}"] .mg-text-mentee{{font-family:{font};font-size:{text_size};font-weight:600;
...
def _render_gsap(params: dict, pack: dict, card_id: str, start: float, end: float) -> list[str]:
    ...
    conn_h = "24px" if pack_id == "lean_ledger" else "32px"
    ...
    if is_cinema:
        ...
        lines.append(f"  tl.fromTo('#{cid}-mg-mentor',{{opacity:0,y:8}},{{opacity:1,y:0,duration:0.32,ease:'circ.out'}},{t_m2:.4f});")
    else:
        lines.append(f"  tl.fromTo('#{cid}-mg-mentor',{{opacity:0,y:6}},{{opacity:1,y:0,duration:0.25,ease:'power2.out'}},{t_m2:.4f});")

    # Connector line GROWS downward (starts at height:0 in CSS)
    if is_cinema:
        lines.append(f"  tl.to('#{cid}-mg-connector',{{opacity:1,height:'{conn_h}',duration:0.60,ease:'power1.in'}},{t_conn:.4f});")
    elif is_ledger:
        lines.append(f"  tl.to('#{cid}-mg-connector',{{opacity:1,height:'{conn_h}',duration:0.15,ease:'none'}},{t_conn:.4f});")
    elif is_craft:
        lines.append(f"  tl.to('#{cid}-mg-connector',{{opacity:1,height:'{conn_h}',duration:0.35,ease:'circ.out'}},{t_conn:.4f});")
    else:
        lines.append(f"  tl.to('#{cid}-mg-connector',{{opacity:1,height:'{conn_h}',duration:0.30,ease:'power1.out'}},{t_conn:.4f});")
    ...
    if is_cinema:
        ...
        lines.append(f"  tl.fromTo('#{cid}-mg-mentee',{{opacity:0,y:8}},{{opacity:1,y:0,duration:0.32,ease:'circ.out'}},{t_a2:.4f});")
    else:
