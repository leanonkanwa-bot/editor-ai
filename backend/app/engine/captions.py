...
def _build_momentum_ass(
    ...
    # Scale by height so portrait (1080x1920) gets the correct on-screen size.
    scale = video_h / 1080.0
    font_size  = max(28, int(64 * scale))
    title_size = max(48, int(120 * scale))
    margin_v   = int(video_h * 0.14)
    margin_lr  = int(80 * scale)

    def _m_hex_to_ass(hex_color: str, alpha: int = 0) -> str:
        h = hex_color.lstrip("#").upper()
        if len(h) != 6:
            h = "FFFFFF"
        r, g, b = h[0:2], h[2:4], h[4:6]
    ...
    if caption_moments:
        for moment in caption_moments:
            ...
            if style_type in ("hook", "stat", "mantra"):
                ...
                anim = r"{\fscx90\fscy90\t(0,200,\fscx100\fscy100)\t(200,1500,\fscx104\fscy104)}"
                lines.append(
                    f"Dialogue: 1,{_ts(start)},{_ts(end)},TitleCard,,0,0,0,,{anim}{text}"
                )
    ...
    i = 0
    while i < len(valid_words):
        group = valid_words[i: i + PHRASE_SIZE]
        if not group:
            break

        start = max(0.0, group[0].start + WHISPER_TIMESTAMP_CORRECTION - 0.05)
        end_base = group[-1].end + WHISPER_TIMESTAMP_CORRECTION

        next_i = i + PHRASE_SIZE
        if next_i < len(valid_words):
            next_start = valid_words[next_i].start + WHISPER_TIMESTAMP_CORRECTION
            gap = next_start - end_base
            end = (end_base + 0.08) if gap > 0.25 else max(start + 0.05, next_start - 0.02)
        else:
            end = end_base + 0.15
        ...
        text = " ".join(parts)
        anim = r"{\fad(30,40)\t(0,90,\fscx108\fscy108)\t(90,160,\fscx100\fscy100)}"
        lines.append(f"Dialogue: 0,{_ts(start)},{_ts(end)},Word,,0,0,0,,{anim}{text}")

        i += PHRASE_SIZE

    output_path.write_text("\n".join(lines), encoding="utf-8")
    print(f"[CAPTIONS] Momentum: {len(lines) - 1} lines written")
    return output_path
