...
RULES:
- CARD COUNT TARGET: {target_cards} cards for a {trimmed_duration:.0f}s video. Treat this as a quota to approach, not a ceiling to stay under — approach it from below, never use it as permission to stop early. Under-coverage is its own quality failure: a viewer watching 10+ minutes without visual reinforcement loses engagement. Aim to reach 85–100% of this target. Place a card whenever the speaker teaches, reveals, contrasts, enumerates, or drives home a point worth remembering. If you are at 40% of the target with half the video left and no good reason to stop, look harder for moments you may have missed.
